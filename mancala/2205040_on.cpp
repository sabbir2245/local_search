...
struct Edge
{
    ...
    // introduce a less overload func here ;
};
...
vector<int> MultigreedyMaxCut(const Graph &g, int T)
{
    int n = g.getN();
    int m = g.getM();
    vector<int> bestpartiton;

    vector<int> partition(n, -1);

    vector<Edge> edgelist(m);

    // sort(edgelist, reverse()); // srt the edges in terms of weight
    ...
    for (int i = 0; i < T; i++)
    {
        ...
        if (computeCutWeight(g, partition) > computeCutWeight(g, bestpartiton))
            bestpartiton = partition;
    }
...
int main()
{
    map<int, string> knownBestMap = {
        {1, "12078"}, {2, "12084"}, {3, "12077"}, {11, "627"}, {12, "621"}, {13, "645"}, {14, "3187"}, {15, "3169"}, {16, "3172"}, {22, "14123"}, {23, "14129"}, {24, "14131"}, {32, "1560"}, {33, "1537"}, {34, "1541"}, {35, "8000"}, {36, "7996"}, {37, "8009"}, {43, "7027"}, {44, "7022"}, {45, "7020"}, {48, "6000"}, {49, "6000"}, {50, "5988"}};
    ...
    csv << "Name,|V| or n,|E| or m,Simple Randomized,Simple Greedy,Multigreedy , Semi-greedy (α = 0.6),Simple Local - No. of iterations,Simple Local - Average value,GRASP (300 iterations) - Best value,Known best solution or upper bound" << endl;
    ...
    for (int i = 0; i < numFiles; i++)
    {
        ...
        cout << "G" << gNum << "\t" << n << "\t" << m << "\t"
             << (int)avgRand << "\t" << greedyW << "\t" << greedyWM << "\t" << semiW << "\t"
