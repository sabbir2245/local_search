...
public:
                           ...
                           const Weights &w)
  {
    ...
    if (depth > 1)
    {
      return 2 * storageDiff(b, player) + sideDiff(b, player);
    }
    else
    {
      return 5 * storageDiff(b, player) + 3 * sideDiff(b, player);
    }
  }
...
public:
                   ...
                   int leadingCaptured)
  {
    ...
    if (depth > 1)
    {
      ...
      return Heuristics::heuristic1(board, currentPlayer, false, 5, TunedWeights::getHeuristic2Weights());
    }
    if (depth == 1)
    {

      return Heuristics::heuristic2(board, currentPlayer, false, 5, TunedWeights::getHeuristic2Weights());
    }

    // we hav eto add depth -- here , and calute it dynamicaly there
